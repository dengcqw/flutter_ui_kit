...
import 'package:ui_kit/widget/my_pageview.dart';
...
class SwiperController extends ChangeNotifier {
  ...
  void _attach(SwiperState state) => _swiperState = state;
...
class _SwiperIndicator implements SwiperIndicator {
  ...
  Widget build(BuildContext context, int index, int itemCount) {
    ...
    var children = List.generate(itemCount, (index1) {
...
class SwiperState extends State<Swiper>
  ...
  Widget build(BuildContext context) {
    var children = <Widget>[];
    children.add(MyPageView.builder(
      ...
      cacheExtent: widget.childCount.toDouble(),
